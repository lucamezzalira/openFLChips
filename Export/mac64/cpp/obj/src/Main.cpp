...
Void Main_obj::buildChips( ){
{
		...
		HX_STACK_LINE(24)
		::flash::display::Bitmap chip;		HX_STACK_VAR(chip,"chip");
		HX_STACK_LINE(25)
		{
			HX_STACK_LINE(25)
			...
			while(((_g < total))){
				...
				HX_STACK_LINE(26)
				chip = ::flash::display::Bitmap_obj::__new(bitmapData,null(),null());
				HX_STACK_LINE(27)
				this->addChild(chip);
				HX_STACK_LINE(29)
...
Void Main_obj::continuosMovement( ::flash::display::Bitmap element){
{
		...
		HX_STACK_LINE(45)
		::motion::Actuate_obj::tween(item,(int)1,_Function_1_1::Block(this,_g,finalYPos),null(),null())->delay((::Math_obj::random() * (int)5))->onComplete(this->continuosMovement_dyn(),Dynamic( Array_obj<Dynamic>::__new().Add(item)));
	}
return null();
}
