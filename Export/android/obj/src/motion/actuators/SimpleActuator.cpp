				...
				if ((::Std_obj::is(start,hx::ClassOf< ::Float >()))){
					struct _Function_4_1{
						inline static Dynamic Block( ::String &i,::motion::actuators::SimpleActuator_obj *__this){
							HX_STACK_FRAME("*","closure",0x5bdab937,"*.closure","motion/actuators/SimpleActuator.hx",188,0x731927e5)
							{
								HX_STACK_LINE(188)
...
Void SimpleActuator_obj::move( ){
{
		...
		if ((::Reflect_obj::hasField(this->properties,HX_CSTRING("alpha")))){
			HX_STACK_LINE(204)
			this->toggleVisible = ::Std_obj::is(this->target,hx::ClassOf< ::flash::display::DisplayObject >());
		}
		else{
			HX_STACK_LINE(204)
			this->toggleVisible = false;
