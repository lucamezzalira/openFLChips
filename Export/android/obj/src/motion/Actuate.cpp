	...
	HX_STACK_ARG(allowCreation,"allowCreation")
{
		...
		if (((  ((!(_Function_1_1::Block(target)))) ? bool(allowCreation) : bool(false) ))){
			HX_STACK_LINE(77)
			::motion::Actuate_obj::targetLibraries->set(target,Array_obj< ::Dynamic >::__new());
		}
		HX_STACK_LINE(81)
...
::motion::actuators::IGenericActuator Actuate_obj::timer( Float duration,::Class customActuator){
	...
	HX_STACK_LINE(275)
	return ::motion::Actuate_obj::tween(_g,duration,::motion::_Actuate::TweenTimer_obj::__new((int)1),false,customActuator);
}
...
{
		HX_STACK_FRAME("motion.Actuate","unload",0x7ce9fdd6,"motion.Actuate.unload","motion/Actuate.hx",366,0xcac92ba8)
...
bool overwrite = __o_overwrite.Default(true);
	HX_STACK_FRAME("motion.Actuate","update",0x9e62e2e0,"motion.Actuate.update","motion/Actuate.hx",395,0xcac92ba8)
	HX_STACK_ARG(target,"target")
	HX_STACK_ARG(duration,"duration")
	...
	HX_STACK_ARG(overwrite,"overwrite")
{
		struct _Function_1_1{
			inline static Dynamic Block( Dynamic &end,Dynamic &start){
				HX_STACK_FRAME("*","closure",0x5bdab937,"*.closure","motion/Actuate.hx",397,0xcac92ba8)
				{
					hx::Anon __result = hx::Anon_obj::Create();
