...
ApplicationMain.preloader_onComplete = function(event) {
	...
	if(hasMain) Reflect.callMethod(Main,Reflect.field(Main,"main"),[]); else {
		...
		if(js.Boot.__instanceof(instance,flash.display.DisplayObject)) flash.Lib.current.addChild(instance); else {
			haxe.Log.trace("Error: No entry point found",{ fileName : "ApplicationMain.hx", lineNumber : 165, className : "ApplicationMain", methodName : "preloader_onComplete"});
			haxe.Log.trace("If you are using DCE with a static main, you may need to @:keep the function",{ fileName : "ApplicationMain.hx", lineNumber : 166, className : "ApplicationMain", methodName : "preloader_onComplete"});
		}
	}
};
...
Main.prototype = $extend(flash.display.Sprite.prototype,{
	buildChips: function() {
		var total = 100;
		var bitmapData = openfl.Assets.getBitmapData("assets/chip.png");
		var chip;
		var _g = 0;
		while(_g < total) {
			var i = _g++;
			chip = new flash.display.Bitmap(bitmapData);
			this.addChild(chip);
			chip.x = Math.random() * this.stage.stageWidth;
			chip.y = 540;
			this.continuosMovement(chip);
		}
	}
	,continuosMovement: function(element) {
		var item = element;
		var finalYPos = 0;
		if(item.y == 0) finalYPos = 540;
		motion.Actuate.tween(item,1,{ x : Math.random() * this.stage.stageWidth, y : finalYPos}).delay(Math.random() * 5).onComplete($bind(this,this.continuosMovement),[item]);
	}
	,__class__: Main
});
...
var DefaultAssetLibrary = function() {
	...
	this.addExternal("assets/chip.png","image","assets/chip.png");
};
...
motion.easing.Back.__name__ = true;
...
motion.easing.BackEaseIn.__name__ = true;
...
motion.easing.BackEaseInOut.__name__ = true;
...
motion.easing.BackEaseOut.__name__ = true;
